// Binary tree is a struct that the nodes can have maximus two childs
// Number of nodes
let N = 7, Root = 1;
...
function addEdge(x:number, y:number, arr:number[][]) {
    arr[x].push(y);
    arr[y].push(x);
}
 
// Function to print the parent of each node
function printParents(node:number, arr:number[][], parent:number)
{
...
function printChildren(Root:number, arr:number[][])
{
...
function printLeafNodes(Root:number, arr:number[][])
{
...
function printDegrees(Root:number, arr:number[][]) {
    for (let i = 1; i < arr.length; i++) {
        console.log(`${i}: `);
